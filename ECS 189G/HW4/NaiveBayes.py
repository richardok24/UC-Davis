...
# Reference: Textbook, Piazza, and discussion slides
...
class NaiveBayes:
    ...
    def __init__(self):
        ...
        self.N = collections.defaultdict(lambda: 0) # Collections of documents with all classes
        self.N['neg'] = 0 # Total # of documents with class negative
        self.N['pos'] = 0 # Total # of documents with class positive
        self.Ndoc = 0 # Total # of documents
        self.Nw = collections.defaultdict(lambda: 0) # Collections of negative words
        self.Pw = collections.defaultdict(lambda: 0) # Collections of positive words
        self.T = set() # set of total words
        self.TNw = set() # set of total negative words
        self.TPw = set() # set of total positive words
    ...
    def classify(self, words):
        ...
        # P(negative) and P(positive)
        Pn = math.log(self.N['neg']) - math.log(self.Ndoc)
        Pp = math.log(self.N['pos']) - math.log(self.Ndoc)
        
        # Negative and positive word counts
        Nc = sum(self.Nw.values())
        Pc = sum(self.Pw.values())
        
        # Binary Naive Bayes or Binary Naive Bayes with improvements
        if self.naiveBayesBool:
            a = 20 # alpha value
            for w in words:
                # Probability of sentiment being negative
                Pn += math.log(self.Nw[w] + a)
                Pn -= math.log(Nc + a * len(self.T))
                
                # Probability of sentiment being positive
                Pp += math.log(self.Pw[w] + a)
                Pp -= math.log(Pc + a * len(self.T))
            ...
            for w in words:
                # Probability of sentiment being negative
                Pn += math.log(self.Nw[w] + 1)
                Pn -= math.log(Nc + len(self.T))
                
                # Probability of sentiment being positive
                Pp += math.log(self.Pw[w] + 1)
                Pp -= math.log(Pc + len(self.T))
        
        # Compare and determine sentiment
        if Pn > Pp:
            return 'neg'
        else:
            return 'pos'
