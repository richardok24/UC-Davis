...
class SpellCorrect:
  ...
  def correctSentence(self, sentence):
    ...
    for i in xrange(1, len(sentence) - 1): #ignore <s> and </s>
      ...
      # Edits and log-probabilities
      editLog = self.editModel.editProbabilities(sentence[i])
      for edit in editLog:
        trialSentence = sentence[:i] + [edit[0]] + sentence[i+1:]
        # Sum of log-probability and edit probability
        currentScore = self.languageModel.score(trialSentence) + edit[1]
        # Condition
