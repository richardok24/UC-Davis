...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for i in xrange(1, len(word) + 1):
      for character in EditModel.ALPHABET:
        #The corrupted signal is the character preceding
        corruptLetters = word[i-1]
        #The correct signal is the preceding character plus the new character
        correctLetters = corruptLetters + character
            
        #The corrected word inserts new character (and lacks the start symbol)
        correction = "%s%s%s" % (word[1:i], character, word[i:])
        ret.append(Edit(correction, corruptLetters, correctLetters))
  ...
  def transposeEdits(self, word):
    ...
    for i in xrange(1, len(word)):
      #The corrupted signal are this character and the character preceding
      corruptLetters = word[i-1:i+1] 
      #The correct signal is the swapped corrupted signal
      correctLetters = corruptLetters[::-1]
        
      #The corrected word swaps characters (and lacks the start symbol)
  ...
  def replaceEdits(self, word):
    ...
    for i in xrange(0, len(word)):
      for character in EditModel.ALPHABET:
        #The corrected word replaces i with new character (and lacks the start symbol)
        correction = word[:i] + character + word[i+1:]
        if (correction != word):
          ret.append(Edit(correction, word[i], character))
