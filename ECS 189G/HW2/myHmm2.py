...
# Richard Xie
# 2/12/18
# An Add-k smoothing method is added to smooth the transition probabilities
...
# Reference: https://www.youtube.com/watch?v=GwP8gKa-ij8

# Here, k is a constant set to 1, and V is the word count
k = 1
V = len(vocab)

for prevtag in transitions:
	for tag in transitions[prevtag]:
		print "trans %s %s %s" % (prevtag, tag, (float(transitions[prevtag][tag]) + k) / (transitionsTotal[prevtag] + k * V))

for tag in emissions:
	for token in emissions[tag]:
		print "emit %s %s %s " % (tag, token, (float(emissions[tag][token]) + k) / (emissionsTotal[tag] + k * V))
